...
import { FLUXOS_INSTAGRAM, badgeEstadoFluxo, formatarData, type FluxoInstagram } from "./_data";
import { DmEditorDialog } from "./dm-editor-dialog";
import { cn } from "@/lib/utils";

const TRIGGERS = getTriggersForChannel("instagram");

export default function FluxosInstagramPage() {
  const router = useRouter();
  const [fluxos, setFluxos] = useState<FluxoInstagram[]>(FLUXOS_INSTAGRAM);
  const [criarOpen, setCriarOpen] = useState(false);
  const [nome, setNome] = useState("");
  const [triggerId, setTriggerId] = useState(TRIGGERS[0]?.id ?? "");
  const [deleteTarget, setDeleteTarget] = useState<FluxoInstagram | null>(null);
  ...
  function handleDuplicar(fluxo: FluxoInstagram) {
    const copia: FluxoInstagram = {
      ...fluxo,
      id: `ig-${Date.now()}`,
  ...
  function handleCriar() {
    ...
    router.push(`/dashboard/fluxos/instagram/${novo.id}`);
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-semibold">Fluxos de Instagram</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Automatiza comentários, menções e DMs no Instagram.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Dialog open={criarOpen} onOpenChange={setCriarOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Criar fluxo de Instagram</DialogTitle>
                <DialogDescription>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="nome-fluxo-ig">Nome do fluxo</Label>
                  <Input
                    id="nome-fluxo-ig"
                    placeholder="Ex: Boas-vindas a novos seguidores"
                    value={nome}
              ...
              return (
                <TableRow key={fluxo.id}>
                  <TableCell className="font-medium">
                    <Link href={`/dashboard/fluxos/instagram/${fluxo.id}`} className="hover:text-primary">
                      {fluxo.nome}
                    </Link>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          render={
                            <Link href={`/dashboard/fluxos/instagram/${fluxo.id}`}>
      ...
      {fluxos.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center text-sm text-muted-foreground">
            Ainda não tens fluxos de Instagram. Cria o primeiro para começar a automatizar.
          </CardContent>
        </Card>
      )}
