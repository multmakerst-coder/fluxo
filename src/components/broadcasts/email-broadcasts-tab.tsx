...
import { badgeEstadoBroadcast, type BroadcastRow } from "@/app/dashboard/broadcasts/_data";
import { NovoBroadcastEmailDialog } from "@/components/broadcasts/novo-broadcast-email-dialog";
import { BroadcastRelatorioDialog } from "@/components/broadcasts/broadcast-relatorio-dialog";
import { toast } from "sonner";

export function EmailBroadcastsTab() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await fetch("/api/broadcasts?channel=email");
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <NovoBroadcastEmailDialog onCriado={(novo) => setBroadcasts((prev) => [novo, ...prev])} />
      </div>
      ...
      {!isLoading && broadcasts.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center text-sm text-muted-foreground">
            Ainda não tens broadcasts de email. Cria o primeiro para começar.
          </CardContent>
        </Card>
      )}
